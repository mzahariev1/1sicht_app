...
class HomeEmployeeActivity : BaseActivity() {
    ...
    private fun getAll() {
        ...
        homeEmployeeViewModel.getEmployee().observe(this, Observer<Employee> { employee ->
            if (employee!=null){
                homeEmployeeViewModel.getAllReviewsForEmployee(employee.id!!).observe(this, Observer<List<Review>> { s ->
                    s.forEach {
                        reviews.add(it)
                    }
                })
    ...
    private fun createAdapterForReviews(reviews: ArrayList<Review>) {
        ...
        val mListener = object: ReviewAdapter.OnItemClickListener{
            override fun onDeleteClick(position: Int) {
                ...
                getAll()
                Toast.makeText(this@HomeEmployeeActivity,getString(R.string.hm_toast),Toast.LENGTH_LONG).show()
            }
