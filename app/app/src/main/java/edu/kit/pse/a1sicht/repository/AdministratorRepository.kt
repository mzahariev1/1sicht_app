 ...
 */
class AdministratorRepository constructor(localDatabase: LocalDatabase, private val adminService: AdministratorService){
    ...
    fun createAdmin(googleID: String, firstName: String, lastName: String): Administrator? {
        ...
        val newAdmin = adminService.create(Administrator(null,googleID,firstName,lastName)).blockingFirst()
    ...
    fun updateAdminById(adminId: Int, admin: Administrator): Administrator {
        ...
        if (admin.id == adminDao.get().value?.id) {
