...
class ReviewCreationViewModel(application: Application) : AndroidViewModel(application) {
     ...
     */
    fun updateReview(reviewId: Int, newReview: Review) {

        val oldReview = reviewRepository.getReviewById(reviewId)
        val tsYear = newReview.date!!.year
        ...
        timeSlotRepository.getAllForReview(reviewId)!!.forEach {
            if (timeSlot.startTime!!.minutes > 59) {
                ...
                timeSlot = createTimeSlot(review.id!!, tsYear, review.date!!.month, review.date!!.day,
                    timeSlot.startTime!!.hours, timeSlot.startTime!!.minutes, review.timeSlotLength,
                    it.maxStudents)

            } else {
                timeSlot = createTimeSlot(
                    review.id!!, tsYear, review.date!!.month,
                    review.date!!.day, timeSlot.startTime!!.hours, timeSlot.startTime!!.minutes,
                    review.timeSlotLength, it.maxStudents
                )
            }
