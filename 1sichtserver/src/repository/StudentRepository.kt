...
class StudentRepository {
    ...
    fun getReviewsForStudent(studentId: Int): List<Review> {
        return transaction {
            return@transaction ((StudentsTimeslots innerJoin Timeslots) innerJoin Reviews)
                .slice(Reviews.id, Reviews.name, Reviews.location, Reviews.date, Reviews.lengthOfTimeslot,
    ...
    fun getStudentsForReview(reviewId: Int): List<Student>?{
        return transaction{
            return@transaction Students.join(StudentsTimeslots, JoinType.INNER, additionalConstraint = {Students.id eq StudentsTimeslots.studentId})
